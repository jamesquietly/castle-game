            ...
            <>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="p-2 rounded flex-1 border border-white text-white"
                  value={roomToJoin || ""}
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-green-900 text-white p-4">
        <h1 className="text-2xl mb-4">
          Room: {role === "HOST" ? peerId : roomToJoin}
        </h1>
        <div className="mb-8 flex flex-col items-center">
          <h2 className="text-xl font-bold mb-2">Players Connected:</h2>
          <ul className="flex flex-wrap gap-2">
            {players.map((p) => (
              <li key={p.id} className="bg-white/20 px-4 py-2 rounded-full">
    ...
    <div className="flex flex-col min-h-screen bg-green-900 text-white p-4 select-none">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p
            className={`font-bold ${isMyTurn ? "text-yellow-400 animate-pulse" : "text-gray-300"}`}
          >
        ...
        {gameState.phase === "DEALING" ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-center max-w-md">
              Select one card from your hand and one face-up card to swap them.
        ...
        <div className="flex justify-center gap-4 mb-4">
          {me.faceDown.map((c) => (
            <div key={c.id} className="relative">
              ...
              />
              <div className="absolute -top-2 -left-2 bg-gray-800 text-[8px] p-1 rounded">
                FD
              </div>
            </div>
          ))}
            ...
            <Card
              ...
              onClick={() => {
                if (gameState.phase === "DEALING") {
                  const handSelected = selectedCards.find((id) =>
                    me.hand.some((hc) => hc.id === id),
                  );
                  if (handSelected) {
        ...
        <div className="flex justify-center -space-x-4">
          {me.hand.map((c) => (
            <Card
              ...
              onClick={() => {
                if (gameState.phase === "DEALING") {
                  const faceUpSelected = selectedCards.find((id) =>
                    me.faceUp.some((fc) => fc.id === id),
                  );
                  if (faceUpSelected) {
