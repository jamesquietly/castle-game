...
import { createDeck, initializeGame, playCards, pickUpPile, swapCards, setReady } from './game-logic';
...
describe('game-logic', () => {
  ...
  it('should throw error when playing lower rank card', () => {
    ...
    expect(() => playCards(state, ['C5'])).toThrow('Invalid move: card rank too low');
  });

  it('should allow picking up the pile', () => {
    const state = initializeGame(['Alice', 'Bob']);
    state.phase = 'PLAYING';
    state.discardPile = [{ suit: 'HEARTS', rank: 8, id: 'H8' }, { suit: 'CLUBS', rank: 9, id: 'C9' }];
  ...
  it('should handle 7 (Lower Than Seven) correctly', () => {
    ...
    expect(() => playCards(newState, ['S8'])).toThrow('Invalid move: card rank too low');
